...
class Derevo:
    ...
    def sv(self):
        dop = [self.n, self.w, self.flag, self.value, self.depth] + [self.u, self.v, self.q]
        if self.pred != -1:
            dop += [self.pred.value]
        else:
            dop += [self.pred]
...
def if_game(derevo):
    global SN, lim_hod
    if derevo.depth< lim_hod:
        if (derevo.pole() == np.zeros((SN, SN, SN), dtype=np.int8)).all():
            derevo.flag = False
            print("YES")
            print(derevo.sv())
            return 1
...
def win():
    return random.randint(0, lim_point)
...
def Game():
    ...
    with open('data_test_main2.pickle', 'wb') as ff:
